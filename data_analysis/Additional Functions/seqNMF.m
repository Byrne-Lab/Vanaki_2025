...
%initial reconstrction of X from W and H
...
smoothkernel = ones(1,(2*L)-1);  % kernel for smoothing in factor competition
disp(smoothkernel); % displays the smoothing kernel
smallnum = max(X(:))*1e-6; % A small number to prevent numerical issues in updates
lasttime = 0; %flag to indicate the final iteration

% Calculate initial cost
% Initialize cost vector to track reconstruction error over iterations
cost = zeros(params.maxiter+1, 1);
cost(1) = sqrt(mean((X(:)-Xhat(:)).^2)); % Compute initial reconstruction error
...
for iter = 1 : params.maxiter
    ...
    if (iter == params.maxiter) || ((iter>5) && (cost(iter+1)+params.tolerance)>mean(cost((iter-5):iter)))
        cost = cost(1 : iter+1);  % trim vector % Trim cost vector to actual number of iterations
        lasttime = 1; 
        if iter>1
    ...
    % Compute regularization terms for H update if specified
    if params.lambda>0
    ...
    % % ADD NEW PENALTY FOR OVERLAP BETWEEN H FACTORS, added 8/22/2024
    % % Assumption: H has 2 factors, one for orange and one for blue
    % % If there are more factors, you might need to loop or generalize
    % 
    % % this part i'm playing around with the different vals of penalty
    % % .01, .1,10,20 too small
    % % 40,50,100 way too big
    % overlap_penalty = 25 * (H(1, :) .* H(2, :));  % Penalize overlap
    % H(1, :) = H(1, :) - overlap_penalty;  % Apply penalty to first factor
    % H(2, :) = H(2, :) - overlap_penalty;  % Apply penalty to second factor
    % 
    % % Ensure H doesn't go below zero after penalty
    % H(H < 0) = 0;


    % Shift to center factors to avoid local minima
    if params.shift
        [W, H] = shiftFactors(W, H);  
    ...
    % Update each Wl separately
        Xhat = reconstruct(W, H); % reconstruct X with updated H
        mask = find(params.M == 0); % find masked (held-out) indices 
        X(mask) = Xhat(mask); % replace data at masked elements with reconstruction, so masked datapoints do not effect fit
...
% power = ( sum x^2 - (sum (x-xhat)^2) ) / sumx^2
% magnitude -  mse / magnitude
power = (sum(X(:).^2)-sum((X(:)-Xhat(:)).^2))/sum(X(:).^2);  % fraction power explained by whole reconstruction
[loadings,ind] = sort(computeLoadingPercentPower(X,W,H),'descend'); % fraction power explained by each factor
